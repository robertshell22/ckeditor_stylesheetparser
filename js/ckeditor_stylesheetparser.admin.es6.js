/**
 * @file
 * CKEditor Stylesheet Parser admin behavior.
 */

(function($, Drupal, drupalSettings, _) {
  /**
   * Ensures that the "stylesheetparser" button's metadata remains up-to-date.
   *
   * Triggers the CKEditorPluginSettingsChanged event whenever the "stylesheetparser"
   ...
   *   Attaches admin behaviour to the "stylesheetparser" button.
   */
  Drupal.behaviors.ckeditorStylesheetParserSettings = {
    attach(context) {
      ...
      const that = this;
      $context
          .find('[name="editor[settings][plugins][stylesheetparser][styles]"]')
          .on('blur.ckeditorStylesheetParserSettings', function() {
     ...
     *
     * @see \Drupal\ckeditor\Plugin\ckeditor\plugin\StylesheetParser::generateStylesSetSetting()
     *
    ...
    _generateStylesSetSetting(styles) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        // Build the data structure CKEditor's stylesheetparser plugin expects.
  ...
  Drupal.behaviors.ckeditorStylesComboSettingsSummary = {
    attach() {
      $('[data-ckeditor-plugin-id="stylesheetparser"]').drupalSetSummary(context => {
        const styles = $.trim(
            $(
                '[data-drupal-selector="edit-editor-settings-plugins-stylesheetparser-styles"]',
            ).val(),
        );
        if (styles.length === 0) {
